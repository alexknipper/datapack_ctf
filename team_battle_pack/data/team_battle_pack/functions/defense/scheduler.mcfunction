...
## Block Crafting
# Effect Field
execute as @a[scores={tbp_nstar_use=1..}] as @e[type=minecraft:item,nbt={Item:{id:"minecraft:nether_star",tag:{display:{Name:'{"text":"Effect Field Core","color":"aqua"}'},HideFlags:39,Enchantments:[{id:"minecraft:infinity",lvl:1}]}},OnGround:1b},distance=..5,limit=1,sort=nearest] at @s unless entity @e[type=minecraft:armor_stand,tag=effect_field,distance=..2,limit=1] if entity @e[type=minecraft:end_crystal,tag=nexus,distance=..2,limit=1] run function team_battle_pack:defense/use/effect_field
# Ranged Turrets
