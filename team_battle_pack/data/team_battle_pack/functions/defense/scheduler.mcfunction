...
# Ranged Turrets
execute as @a[scores={tbp_arrow_use=1..}] at @s if score @s tbp_team = @e[type=minecraft:end_crystal,tag=nexus,distance=..100,limit=1] tbp_team if score @e[type=minecraft:end_crystal,tag=nexus,distance=..100,limit=1] tbp_num_turret_r < limit tbp_num_turret_r as @e[type=minecraft:item,nbt={Item:{id:"minecraft:arrow",tag:{display:{Name:'{"text":"Ranged Turret Kit","color":"yellow"}'},HideFlags:39,Enchantments:[{id:"minecraft:infinity",lvl:1}]}},OnGround:1b},distance=..5,limit=1,sort=nearest] at @s unless entity @e[type=minecraft:armor_stand,tag=ranged_turret,distance=..35] if block ~ ~-.15 ~ minecraft:smooth_stone_slab if entity @e[type=minecraft:end_crystal,tag=nexus,distance=..98,limit=1] run function team_battle_pack:defense/use/ranged_turret
